...
if (isset($_POST['saveButton']) && isset($_POST['markers2'])) {
    // Save markers2 to the database
    $markers = json_decode($_POST['markers2'], true);
    ...
    foreach ($markers as $marker) {
        ...
        $query = "SELECT * FROM markers2 WHERE title = ?";
        ...
        if (mysqli_num_rows($result) > 0) {
            $updateQuery = "UPDATE markers2 SET lat = ?, lng = ? WHERE title = ?";
            ...
            $insertQuery = "INSERT INTO markers2 (title, lat, lng) VALUES (?, ?, ?)";
    ...
    echo "markers2 saved successfully!";
} elseif (isset($_POST['loadButton'])) {
    // Load markers2 from the database
    $markers = array();
...
<body>
    ...
    <script>
        ...
        // Declare markers2 as a global variable
        var markers2 = [];
    ...
    // Create a layer group to store the markers2
...
function resetMarkers() {
    ...
    if (confirmReset) {
        // Remove all markers2 from the marker layer
        markerLayer.clearLayers();

        // Clear markers2 from local storage
        localStorage.removeItem('markers2');

        // Reset markersSaved when markers2 are reset
        markersSaved = false;

        // Reset the button text
        saveButton.textContent = 'Save markers2';
    ...
    resetButton.textContent = 'Reset markers2';
    ...
    resetButton.addEventListener('click', function () {
        ...
        function resetMarkers() {
            // Check if markers2 are saved, and reset markersSaved without prompting if they are
            if (markersSaved) {
                markersSaved = false; // Reset markersSaved when markers2 are reset
                saveButton.textContent = 'Save markers2'; // Reset the button text
            } else {
                var confirmReset = confirm('Are you sure you want to reset the markers2? This action cannot be undone.');
...
// ADD BLUE ICONS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Button to toggle adding markers2
    ...
    markers2.push({ title: title, lat: latlng.lat, lng: latlng.lng });
    });

    // Helper function to find a marker by its coordinates
    function findMarkerByLatLng(latlng) {
        var markers2 = markerLayer.getLayers();
        for (var i = 0; i < markers2.length; i++) {
            if (markers2[i].getLatLng().equals(latlng)) {
                return markers2[i];
            }
        }
        return null;
    ...
    function findMarkerByName(title) {
        var markers2 = markerLayer.getLayers();
        for (var i = 0; i < markers2.length; i++) {
            if (markers2[i].options.title === title) {
                console.log("markers2:", markers2[i]);
                return markers2[i];
            }
        }
        return null;
...
// SAVED BUTTON //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   // Updated variable to track whether markers2 are saved
        var markersSaved = false;

        // Button to save markers2
   ...
   function saveMarkers() {
        ...
        console.log("AJAX URL:", "map2_test.php");
        // AJAX request to save markers2 to the database
        $.ajax({
            type: "POST",
            url: "map2_test.php", // Replace with the actual filen ame
            data: { saveButton: true, markers2: JSON.stringify(markers2) },
            success: function (response) {
                alert(response); // Display the server response
                // Optionally, update the button text or perform other actions
            },
            error: function (xhr, status, error) {
                console.error("Error saving markers2:", error);
...
// DATABASE ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   function updateMarkerInDatabase(title, lat, lng) {
        ...
        $.ajax({
            type: "POST",
            url: "map2_update_marker.php", // Replace with the actual filename or endpoint
    ...
    function deleteMarkerInDatabase(title, lat, lng) {
        ...
        $.ajax({
            type: "POST",
            url: "map2_delete_marker.php", // Replace with the actual filename or endpoint
   ...
   // Load markers2 from local storage if available, only if markers2 are not saved
   var storedMarkers = localStorage.getItem('markers2');
   if (storedMarkers && !markersSaved) {
        var confirmReload = confirm('Do you want to load the saved markers2?');
        if (confirmReload) {
   ...
   function updateMarkersInStorage() {
        ...
        var markers2 = markerLayer.getLayers().map(function (marker) {
                return {
        ...
        window.onbeforeunload = function (event) {
            if (markerLayer.getLayers().length > 0) {
                return 'You have unsaved markers2. Do you really want to leave?';
            }
   };

   // Load markers2 from the database
    function loadMarkersFromDatabase() {
        // AJAX request to load markers2 from the database
        $.ajax({
            type: "POST",
            url: "map2_load_marker.php",
            ...
            success: function (response) {
                ...
                try {
                    ...
                    if (databaseMarkers.length > 0) {
                        ...
                        console.log("No markers2 returned from the database.");
                    }

                    // Add markers2 to the map
            ...
            error: function (xhr, status, error) {
                console.error("Error loading markers2 from the database:", error);
                console.log(xhr.responseText); // Log the full response for debugging
            }
        });
    }
...
// Call the function to load markers2 from the database
