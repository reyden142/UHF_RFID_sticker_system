...
if (isset($_POST['saveButton']) && isset($_POST['markers1_200'])) {
    // Save markers1_200 to the database
    $markers = json_decode($_POST['markers1_200'], true);
    ...
    foreach ($markers as $marker) {
        ...
        $query = "SELECT * FROM markers1_200 WHERE title = ?";
        ...
        if (mysqli_num_rows($result) > 0) {
            $updateQuery = "UPDATE markers1_200 SET lat = ?, lng = ? WHERE title = ?";
            ...
            $insertQuery = "INSERT INTO markers1_200 (title, lat, lng) VALUES (?, ?, ?)";
    ...
    echo "markers1_200 saved successfully!";
} elseif (isset($_POST['loadButton'])) {
    // Load markers1_200 from the database
    $markers = array();

    $query = "SELECT * FROM markers1_200";
...
<body>
    ...
    <script>
        ...
        // Declare markers1_200 as a global variable
        var markers1_200 = [];

        console.log("markers1_200:", markers1_200);
    ...
    // Create a layer group to store the markers1_200
...
function resetMarkers() {
    ...
    if (confirmReset) {
        // Remove all markers1_200 from the marker layer
        markerLayer.clearLayers();

        // Clear markers1_200 from local storage
        localStorage.removeItem('markers1_200');

        // Reset markersSaved when markers1_200 are reset
        markersSaved = false;

        // Reset the button text
        saveButton.textContent = 'Save markers1_200';
    ...
    resetButton.textContent = 'Reset markers1_200';
    ...
    resetButton.addEventListener('click', function () {
        ...
        function resetMarkers() {
            // Check if markers1_200 are saved, and reset markersSaved without prompting if they are
            if (markersSaved) {
                markersSaved = false; // Reset markersSaved when markers1_200 are reset
                saveButton.textContent = 'Save markers1_200'; // Reset the button text
            } else {
                var confirmReset = confirm('Are you sure you want to reset the markers1_200? This action cannot be undone.');
...
// ADD BLUE ICONS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Button to toggle adding markers1_200
    ...
    markers1_200.push({ title: title, lat: latlng.lat, lng: latlng.lng });
    });

    // Helper function to find a marker by its coordinates
    function findMarkerByLatLng(latlng) {
        var markers1_200 = markerLayer.getLayers();
        for (var i = 0; i < markers1_200.length; i++) {
            if (markers1_200[i].getLatLng().equals(latlng)) {
                return markers1_200[i];
            }
        }
        return null;
    }
    ...
    function findMarkerByName(title) {
        var markers1_200 = markerLayer.getLayers();
        for (var i = 0; i < markers1_200.length; i++) {
            if (markers1_200[i].options.title === title) {
                console.log("markers1_200:", markers1_200[i]);
                return markers1_200[i];
            }
        }
        return null;
...
// SAVED BUTTON //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   // Updated variable to track whether markers1_200 are saved
        var markersSaved = false;

        // Button to save markers1_200
   ...
   function saveMarkers() {
        ...
        console.log("AJAX URL:", "map1_200_test.php");
        // AJAX request to save markers1_200 to the database
        $.ajax({
            type: "POST",
            url: "map1_200_test.php", // Replace with the actual filen ame
            data: { saveButton: true, markers1_200: JSON.stringify(markers1_200) },
            success: function (response) {
                alert(response); // Display the server response
                // Optionally, update the button text or perform other actions
            },
            error: function (xhr, status, error) {
                console.error("Error saving markers1_200:", error);
...
// DATABASE ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   function updateMarkerInDatabase(title, lat, lng) {
        ...
        $.ajax({
            type: "POST",
            url: "map1_200_update_marker.php", // Replace with the actual filename or endpoint
    ...
    function deleteMarkerInDatabase(title, lat, lng) {
        ...
        $.ajax({
            type: "POST",
            url: "map1_200_delete_marker.php", // Replace with the actual filename or endpoint
   ...
   // Load markers1_200 from local storage if available, only if markers1_200 are not saved
   var storedMarkers = localStorage.getItem('markers1_200');
   if (storedMarkers && !markersSaved) {
        var confirmReload = confirm('Do you want to load the saved markers1_200?');
        if (confirmReload) {
   ...
   function updateMarkersInStorage() {
        ...
        var markers1_200 = markerLayer.getLayers().map(function (marker) {
                return {
                    lat: marker.getLatLng().lat,
                    lng: marker.getLatLng().lng
                };
            });
            localStorage.setItem('markers1_200', JSON.stringify(markers1_200));
        }

        window.onbeforeunload = function (event) {
            if (markerLayer.getLayers().length > 0) {
                return 'You have unsaved markers1_200. Do you really want to leave?';
            }
   };

   // Load markers1_200 from the database
    function loadMarkersFromDatabase() {
        // AJAX request to load markers1_200 from the database
        $.ajax({
            type: "POST",
            url: "map1_200_load_marker.php",
            ...
            success: function (response) {
                ...
                try {
                    ...
                    if (databaseMarkers.length > 0) {
                        ...
                        console.log("No markers1_200 returned from the database.");
                    }

                    // Add markers1_200 to the map
            ...
            error: function (xhr, status, error) {
                console.error("Error loading markers1_200 from the database:", error);
                console.log(xhr.responseText); // Log the full response for debugging
            }
        });
    }
...
// Call the function to load markers1_200 from the database
